...
The term _dataset shift_ was probably coined by [Storkey 2009](#references) and means that the training and
...
- _Sample selection bias_: Samples may be discarded from the training with a certain, possibly unknown probability.
- _Changing environments_: The mechanism that produces the data changes between collecting
...
Probably the most well-known shift is the _covariate shift_ where the marginal distribution of the inputs changes, but the conditional
...
This 'difference in data-distribution' is referred to as _dataset shift_. There are
_4 types of shifts_ which each describe a special case where one distribution of the data-generating process
...
- _The type of shift often shouldn't matter:_ A predictive model probably does not care
...
- _The type of shift should matter:_ It is often not clear
...
[1] Storkey A.J. 2009 _When Training and Test Sets are Different: Characterising Learning Transfer_,
in _Dataset Shift in Machine Learning_ MIT Press, pages 3-28.

[2] Moreno-Torres et al. 2012 _A unifying view on dataset shift in classification_,
Pattern Recognition 45, pages 521–530.

[3] Quionero-Candela et al. 2009 _Dataset Shift in Machine Learning_,
The MIT Press.
...
_Feature importance:_ Consider e.g., covariate shift. If an unimportant feature shifts, the generalisation
...
_Shift type:_ During deployment, often only $$P(x)$$ is available for a new dataset, and the targets $$y$$ are missing.
...
_Model assumptions:_ There is an interplay between dataset, shift appearance, and machine learning model.
