...
We will call $$p_t^{\mathrm{newton}}$$ the _Newton direction_ or _Newton step_.
...
Two famous families are the _Broyden_ and the _Dennis_ family of quasi-Newton methods the latter of which contains
...
The _quasi-Newton step_ is then defined as
...
This idea is encoded in the _secant equation_.
...
which means that $$B_t$$ is equal to the _average_ Hessian along the sub-space $$s_{t-1}$$.
...
$$W$$, quasi-Newton methods are also denoted as _variable metric_ methods in the literature.

Retrieving $$B_{t+1}$$ from Eq. \eqref{eq:qn} is a bit tedious (proof can be found in the [reference](#references)).
The important point is that the solution $$B_{t+1}$$ is _analytic_, and, for some vector $$c_t := Ws_t\in\mathbb{R}^d$$ that is
...
Eq. \eqref{eq:dennis} describes the Hessian estimates of the _Dennis family_ of quasi-Newton methods [[2]](#references) which is
...
The simple trick of _limited memory_ quasi-Newton methods such as _limited memory BFGS_ (L-BFGS)
...
its initial iterates, they play an _integral part_ in quasi-Newton methods and cannot be separated from them.
...
the _conjugate gradient algorithm_ (CG) which coincides with BFGS in some sense under certain circumstance.
...
[1] J. J. Dennis and J. Moré 1977 _Quasi-Newton methods, motivation and theory._, SIAM Review 19.1 pp. 46–89.

[2] J. Dennis 1971 _On some methods based on Broyden’s secant approximations._, Numerical Methods for Non-Linear Optimization.

[3] C. Broyden 1969 _A new double-rank minimization algorithm_, Notices of the AMS 16.4 p. 670.

[4] R. Fletcher 1970 _A new approach to variable metric algorithms_, The Computer Journal 13.3 p. 317.

[5] D. Goldfarb 1970 _A family of variable metric updates derived by variational means_, Math. Comp. 24.109 pp. 23–26.

[6] D. Shanno 1970 _Conditioning of quasi-Newton methods for function minimization_, Math. Comp. 24.111 pp. 647–656.

[7] J. Greenstadt 1970 _Variations on variable-metric methods_, Math. Comp 24 pp. 1–22.

[8] M. Powell 1970 _A new algorithm for unconstrained optimization_, Nonlinear Programming.

[9] W. Davidon 1959 _Variable metric method for minimization_, Tech. rep. Argonne National Laboratories, Ill.

[10] R. Fletcher and M. Powell 1963 _A rapidly convergent descent method for minimization_ The Computer Journal 6.2 pp. 163–168.
